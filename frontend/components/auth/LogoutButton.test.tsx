...
describe("LogoutButton", () => {
  ...
  it("surfaces an inline error and does not redirect on 500", async () => {
    ...
    render(<LogoutButton />);
    await user.click(screen.getByRole("button", { name: /log out/i }));
    await waitFor(() => expect(screen.getByRole("alert")).toHaveTextContent(/could not log out/i));
  ...
  it("surfaces an inline error on network failure", async () => {
    server.use(http.post(`${getApiBaseUrl()}/auth/logout`, () => HttpResponse.error()));
    const user = userEvent.setup();
    render(<LogoutButton />);
    await user.click(screen.getByRole("button", { name: /log out/i }));
    await waitFor(() => expect(screen.getByRole("alert")).toHaveTextContent(/could not log out/i));
  ...
  it("shows a loading label while logging out", async () => {
    server.use(
      http.post(`${getApiBaseUrl()}/auth/logout`, async () => {
