...
model: a refusal is detected structurally (the canonical off-topic message, with
no tool calls and no citations), so the check is cheap and deterministic.
...
# Leading clause of OFF_TOPIC_MESSAGE, normalized; matched as a substring so an
# EMERGENCY_BANNER prefix never hides it.
...
# Off-topic questions to refuse, plus in-scope controls to answer. The euthanasia
# case is in scope: it should get a vet referral, not an off-topic refusal.
...
def is_refusal(text: str) -> bool:
    """True when the answer text is the scope-guardrail off-topic refusal.
    ...
    return _OFF_TOPIC_FINGERPRINT in normalized
