...
function weightStat(entries: JournalEntryRead[]): JournalStat {
  ...
  const base = {
    ...
    iconType: "weight" as EntryType,
  };
  const weights = entries.filter(
    (
      entry,
    ): entry is JournalEntryRead & { payload: { entry_type: "weight"; weight_grams: number } } =>
...
function medicationStat(entries: JournalEntryRead[], now: Date | null): JournalStat {
  ...
  const subtitle = medication.payload.missed_dose
    ? `Missed · ${recency}`
    : `${medication.payload.dose} · ${recency}`;
  return {
    ...base,
    label,
    value: medication.payload.drug_name,
    unit: "",
    subtitle,
    sparkline: null,
    present: true,
